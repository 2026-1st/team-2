...
이 문서는 Riot API로 만든 LoL 서렌 예측 데이터셋의 기준을 정리한 문서입니다.  
데이터를 수집하거나 feature를 만들 때는 아래 기준을 따릅니다.

## 분석 단위

데이터셋의 한 행은 "한 경기의 한 팀"입니다.

- 조건을 만족하는 경기 1개에서는 두 행을 만듭니다. 블루 팀은 `team_id=100`, 레드 팀은 `team_id=200`입니다.
- 모델에 넣는 feature는 15분 타임라인 프레임 또는 timestamp가 `<= 900000` ms인 이벤트만 사용합니다.
- 라벨은 경기 종료 상태에서 따로 만들고, feature에는 넣지 않습니다.

## 포함 / 제외 규칙

아래 조건에 걸리는 경기는 데이터셋에서 제외합니다.
...
조기 서렌이나 리메이크에 가까운 경기는 서렌 패배로 세지 않고 제외합니다.

### 피처

모든 `*_diff_15` 컬럼은 우리 팀 기준의 차이값입니다.
...
가공 데이터나 공유용 데이터에는 아래 정보를 넣지 않습니다.
...
재현성을 위해 원본 JSON은 gitignore 처리된 `data/raw/riot/` 아래에 로컬로 보관할 수 있습니다. 다만 공개 산출물에는 팀 단위로 가공한 필드만 사용합니다.

## 데이터 분할 규칙

모델 학습용 데이터를 나눌 때는 반드시 `match_id` 기준으로 묶어서 split합니다. 같은 경기에서 나온 블루/레드 행이 train/validation/test에 따로 들어가면 안 됩니다.
